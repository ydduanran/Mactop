...
def manage_clustering_for_clique(mat, inflation=15, power=1):
    mat[mat <= 0] = 0
    for aa in range(np.shape(mat)[0]):
        if mat[aa, aa] == 0:
            mat[aa, aa] = 1
...
def load_config_from_txt(filename):
    ...
    config['chr_num'] = parse_chr_num_from_str(config['chr_num'])
    config['resolution'] = int(config['resolution'])
    config['normalization'] = True if config['normalization'] == "True" else False
    config['inflation'] = float(config['inflation'])
    config['community_inflation'] = float(config['community_inflation'])
    config['window_size'] = int(config['window_size'])
    config['variance'] = float(config['variance'])
    config['stable_cut_of'] = int(config['stable_cut_of'])
    config['consistent'] = True if config['normalization'] == "True" else False
    config['chromunity_inflation'] = float(config['chromunity_inflation'])
    return config


def get_boundary_type(value_list, min_list, config):
    value_cut_off_90 = np.percentile(value_list, 90)
    value_cut_off = np.percentile(value_list, config['stable_cut_of'])
    count_index = []
    result_list = []
...
def get_community_list(result_list, dense_mat,community_inflation):
    tad_list = get_tad_list(result_list)
    ...
    ddre = manage_clustering_for_clique(adj_mat, inflation=community_inflation)
...
def mactop_for_dense_mat(config):
    for chr_num in config['chr_num']:
        if chr_num == 23:
            chr_num = 'X'
        dense_mat_path = "{0}/chr_{1}".format(config['input_path'], chr_num)
        print('load chr_{0} data'.format(chr_num))
        dense_mat = np.loadtxt(dense_mat_path)
        if config['normalization']:
            dense_mat = original_matrix_to_ice_matrix(dense_mat)

        tad_df = pd.DataFrame(columns=['begin', 'begin_type', 'end', 'end_type', 'community'])
        print('call TAD')
        ...
        community_for_each_tad = get_community_list(result_list, dense_mat,config['community_inflation'])
        cf = len(community_for_each_tad)
        bf = len(tad_df['begin'])
        if bf >= cf:
            for index in range(cf):
                tad_df['community'][index] = community_for_each_tad[index]
        if cf > bf:
            for index in range(bf):
                tad_df['community'][index] = community_for_each_tad[index]

        tad_df.to_csv("{0}/chr_{1}_tad.csv".format(config['output_path'], chr_num))


def mactop_for_cool(config):
    filepaths = ('{0}::resolutions/{1}'.format(config['input_path'], config['resolution']))
    clr = cooler.Cooler(filepaths)
    for chr_num in config['chr_num']:
        if chr_num == 23:
            chr_num = 'X'
        print('load chr_{0} data'.format(chr_num))
        dense_mat = clr.matrix(balance=False).fetch('chr{0}'.format(chr_num))
        if config['normalization']:
            dense_mat = original_matrix_to_ice_matrix(dense_mat)
        tad_df = pd.DataFrame(columns=['begin', 'begin_type', 'end', 'end_type', 'community'])
        ...
        if config['consistent']:
            np.savetxt("{0}/chr_{1}_consistentmat".format(config['output_path'], chr_num), consistentmat)
        begin_list, begin_type_list, end_list, end_type_list, result_list, count_index = get_boundary_type(value_list,
                                                                                                           min_list,
                                                                                                           config)
        tad_df['begin'] = begin_list
...
def mactop_for_mutiway(config):
    resolution = config['resolution']
    print('load data')
    ori_reads_data = pd.read_csv(config['input_path'])
    for chr_num in config['chr_num']:
        if chr_num == 23:
            chr_num = 'X'
        print('build matrix')
        dense_mat = get_mat_from_muti_reads(ori_reads_data, chr_num, config['resolution'])
        if config['normalization']:
            dense_mat = original_matrix_to_ice_matrix(dense_mat)
        ...
        select_chr_reads = ori_reads_data[(ori_reads_data['align1_chrom'] == 'chr{0}'.format(chr_num)) & (
                ori_reads_data['align2_chrom'] == 'chr{0}'.format(chr_num))]
        chromunity_tad_list = []
        unchromunity_tad_list = []
        tad_list = get_tad_list(result_list)
        print('call chromunity')
        tad_chromunity = {}
        for tad_indexs in range(len(tad_list)):
            ...
            if concatemers_copy[['align1_start', 'align2_start']].empty:
                unchromunity_tad_list.append(tad_indexs)
                continue
            max_number = concatemers_copy[['align1_start', 'align2_start']].max(axis=1).max().max()
            ...
            clusters = mc.get_clusters(mc.run_mcl(mc_copy, inflation=config['chromunity_inflation']))
            cris = []
            cluster_length_list = []
            for cc in clusters:
            ...
            if len(cris) > 1:
                chromunity_tad_list.append(tad_indexs)
            else:
                unchromunity_tad_list.append(tad_indexs)
        ...
        with open("{0}/chr_{1}_tad_chromunity.pkl".format(config['output_path'], chr_num), 'wb') as f:
            pickle.dump(tad_chromunity, f)


if __name__ == '__main__':
    config_path = sys.argv[1]
    config = load_config_from_txt(config_path)
    if config['dtype'] == 'dense':
        mactop_for_dense_mat(config)
    if config['dtype'] == 'cool' or config['dtype'] == 'mcool':
        mactop_for_cool(config)
    if config['dtype'] == 'mutiway':
        mactop_for_mutiway(config)
